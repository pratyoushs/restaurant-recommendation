...
public interface UserRepository extends GraphRepository<User> {
    ...
    @Query("MATCH (u:User)-[:Friend_Of]->(u2:User) WHERE (u.email={0}) " +
            "RETURN u2.firstName as firstName, u2.lastName as lastName, u2.email as email, " +
            "u2.gender as gender, true as isFriend")
    List<UserDetails> getAllFriends(String email);

    @Query("MATCH (u:User),(u2:User) WHERE NOT((u)-[:Friend_Of]->(u2)) AND u2.userId <> {0} AND u.email={0} " +
            "RETURN u2.firstName as firstName, u2.lastName as lastName, u2.email as email, " +
            "u2.gender as gender, false as isFriend")
    List<UserDetails> getAllOtherUsers(String email);

    @Query("MATCH (u:User),(u2:User) WHERE u.email={0} AND u2.email={1} " +
            "MERGE (u)-[:Friend_Of]->(u2)")
    void addFriend(String email1, String email2);

    @Query("MATCH (u:User)-[f:Friend_Of]->(u2:User) WHERE u.email={0} AND u2.email={1} " +
            "DELETE f")
    void deleteFriend(String email1, String email2);
